...
- [ ] Flyway V3：站内通知表 + 通知已读状态
- [ ] `NotificationService` 接口 + Spring Event 事件驱动（清单生成/认领/完成触发）
- [ ] 通道实现：微信一次性订阅消息（需 🧑 在小程序后台开通模板）
- [ ] 通道实现：飞书 Webhook、Bark/Server酱
- [ ] Web 端兜底：轮询 + 站内信页
- [ ] 通道开关配置化（deploy/.env）+ 测试 + 部署验证
